...
namespace Security.Application.Implementations
{
    public class AuthHelper : IAuthHelper
    {
        ...
        public Security.Domain.DTO.User.UserInfo GetCurrentUserInfo()
        {

            if (_contextAccessor.HttpContext.User.Claims.FirstOrDefault() == null)
            {
                return new Security.Domain.DTO.User.UserInfo();
            }
            ...
            return new Security.Domain.DTO.User.UserInfo { FullName = FullName, RoleID = RoleID, RoleName = RoleName, UserID = UserID, UserName = UserName, Email = Email, Mobile = Mobile };
        }

        public void Signin(Security.Domain.DTO.User.UserInfo UserInfo)
        {
